...
const SUPPORT = 'support@design-sparring.org'

const base = (content) => `
  <!DOCTYPE html><html>
  <body style="font-family:monospace;background:#F9F7F2;padding:48px;color:#1A1714;">
  <div style="max-width:480px;margin:0 auto;">
    <p style="font-size:12px;letter-spacing:0.12em;text-transform:uppercase;color:#918D87;margin-bottom:32px;">Design Sparring</p>
    ${content}
    <p style="font-size:12px;color:#918D87;margin-top:40px;line-height:1.6;">
      Questions? <a href="mailto:${SUPPORT}" style="color:#C63B15;">${SUPPORT}</a>
    </p>
  </div>
  </body></html>
`
...
export async function sendMagicLink(email, token) {
  ...
  await resend.emails.send({
    ...
    html: base(`
      ...
      <a href="${url}" style="display:inline-block;background:#1A1714;color:#F9F7F2;text-decoration:none;font-family:monospace;font-size:12px;letter-spacing:0.1em;text-transform:uppercase;padding:14px 28px;">Sign in</a>
    `),
  })
}
...
export async function sendPaymentFailedEmail(email) {
  ...
  await resend.emails.send({
    ...
    html: base(`
      ...
      <a href="${portalUrl}" style="display:inline-block;background:#C63B15;color:white;text-decoration:none;font-family:monospace;font-size:12px;letter-spacing:0.1em;text-transform:uppercase;padding:14px 28px;">Update payment method</a>
    `),
  })
}
...
export async function sendPaymentRecoveredEmail(email) {
  await resend.emails.send({
    from: FROM, to: email,
    subject: 'Payment recovered — you are all set',
    html: base(`
      <h1 style="font-family:sans-serif;font-size:28px;font-weight:700;margin-bottom:16px;letter-spacing:-0.02em;">You are back.</h1>
      <p style="font-size:15px;color:#3A3733;margin-bottom:32px;line-height:1.7;">Your payment was processed successfully. Your Sparring Partner access is fully active.</p>
    `),
  })
}