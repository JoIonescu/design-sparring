...
- Stay strictly in character at all times. You do not help with anything other than arguing against design decisions.`;
...
.ds-session-pips { display: flex; gap: 3px; }
.ds-session-pip { width: 14px; height: 3px; background: var(--border); }
.ds-session-pip-on { background: var(--ink); }
...
.spar-verdict-pips { display: flex; gap: 5px; }
.spar-verdict-pip { width: 24px; height: 4px; background: var(--border); }
.spar-verdict-pip-on { background: var(--ink); }
...
@media (max-width: 768px) {
  ...
  .ds-hero { padding: 52px 20px 48px; }
  .ds-hero-inner { grid-template-columns: 1fr; gap: 44px; }
  .ds-section { padding: 60px 20px; }
  .ds-steps { grid-template-columns: 1fr; gap: 36px; }
  .ds-testi-grid { grid-template-columns: 1fr; gap: 36px; }
  .ds-pricing { grid-template-columns: 1fr; }
  .ds-footer { padding: 24px 20px; flex-direction: column; gap: 16px; text-align: center; }
  .ds-score-row { flex-direction: column; align-items: flex-start; gap: 12px; }
  .ds-modal { padding: 36px 24px; }
  .ds-legal { padding: 48px 20px 72px; }
  .ds-cookie { padding: 16px 20px; flex-direction: column; align-items: flex-start; gap: 16px; }
  .spar-page { padding: 32px 20px 80px; }
...
async function callSpar(messages, system) {
...
/* ============ SPARRING INTERFACE ============ */
function SparringInterface({ user, onExit, onSignup, showToast }) {
  const isPaid = user?.plan === "paid";
  const maxRounds = isPaid ? 3 : 1;

  const [status, setStatus] = useState("input"); // input | fighting | wall | verdict
  const [rationale, setRationale] = useState("");
  const [exchanges, setExchanges] = useState([]); // [{role:'user'|'counter', text, round}]
  ...
  const startFight = async () => {
    if (!rationale.trim() || rationale.trim().length < 15) {
      setError("Give the opponent something to work with. Describe your decision more fully.");
      return;
    }
    setError(null);
    setLoading(true);
    setStatus("fighting");
    try {
      const text = await callSpar([{ role: "user", content: `Design decision: ${rationale}` }], SPAR_SYSTEM);
  ...
  const submitDefense = async () => {
    ...
    try {
      ...
      if (nextRound >= maxRounds) {
        // Get final counter + request verdict
        const counterText = await callSpar(msgs, SPAR_SYSTEM);
        ...
        const verdictMsgs = [{ role: "user", content: `Here is a design sparring session. Deliver the verdict.\n\nDesigner's rationale: ${rationale}\n\n${finalExchanges.map(e => `${e.role === "counter" ? "Counter" : "Designer"} (Round ${e.round}): ${e.text}`).join("\n\n")}` }];
        const verdictRaw = await callSpar(verdictMsgs, VERDICT_SYSTEM);
        try {
          const parsed = JSON.parse(verdictRaw.replace(/```json|```/g, "").trim());
          setVerdict(parsed);
          setStatus("verdict");
        } catch {
          setError("Verdict parsing failed — but the fight was real.");
          setStatus("verdict");
        }
      } else {
        const text = await callSpar(msgs, SPAR_SYSTEM);
  ...
  const pipStatus = (r) => {
    if (roundsDone > r) return "done";
    if (roundsDone === r || (r === 0 && status === "fighting")) return "active";
    return "";
  };
      ...
      {status === "input" && (
        <div className="spar-prompt">
          ...
          <textarea
            ...
            placeholder="e.g. We decided to use a bottom navigation bar instead of a hamburger menu because users need fast access to the four main sections of the app..."
            value={rationale}
          ...
          <div className="spar-submit-row">
            <button className="ds-btn-primary" onClick={startFight} disabled={rationale.trim().length < 15}>
              Enter the ring →
            </button>
          </div>
        ...
        <>
              ...
              <div className="spar-turn" key={i}>
                {ex.role === "counter" ? (
                  <>
                    ...
                    <div className="spar-counter-body">
                      <p className="spar-turn-text spar-turn-text-counter">{ex.text}</p>
                    </div>
                  </>
                ) : (
                  <>
          ...
          {loading && (
            <div className="spar-loading">
              <div className="spar-loading-dots">
                <div className="spar-loading-dot" /><div className="spar-loading-dot" /><div className="spar-loading-dot" />
              </div>
          ...
          {/* Defense input — shown after each counter if there are rounds remaining and not at wall */}
          {!loading && status === "fighting" && roundsDone < maxRounds && exchanges[exchanges.length - 1]?.role === "counter" && (
            <div className="spar-defense">
              <span className="spar-defense-lbl">
                {roundsDone < maxRounds - 1 ? `Defend your position — Round ${roundsDone + 1}` : `Final defense — Round ${roundsDone + 1}`}
              </span>
              <textarea
                className="spar-textarea"
              ...
              />
              <div className="spar-submit-row">
                <button className="ds-btn-primary" onClick={submitDefense} disabled={!defenseInput.trim()}>
                  {roundsDone < maxRounds - 1 ? "Push back →" : "Final answer →"}
                </button>
              </div>
            </div>
          )}
          ...
          {status === "wall" && (
            <div className="spar-wall">
              ...
              <p className="spar-wall-sub">
                The free tier stops here. Upgrade to Sparring Partner for two more rounds and a scored verdict — the part that actually tells you something.
              </p>
          ...
          {status === "verdict" && verdict && (
            <div className="spar-verdict">
              <div className="spar-verdict-header">
                <h3 className="spar-verdict-title">The Verdict</h3>
                <p className="spar-verdict-sub">After three rounds, here's how your argument held up.</p>
              </div>
              <div className="spar-verdict-scores">
                {[
                  { label: "Clarity", score: verdict.clarity },
                  { label: "User impact", score: verdict.userImpact },
                  { label: "Defensibility", score: verdict.defensibility },
                ].map(s => (
                  <div className="spar-verdict-row" key={s.label}>
                    <span className="spar-verdict-lbl">{s.label}</span>
                    <Pips score={s.score} total={5} />
                  </div>
                ))}
              </div>
              {verdict.blindSpots?.length > 0 && (
                <div style={{ marginBottom: 24 }}>
                  ...
                  {verdict.blindSpots.map((b, i) => (
                    <p className="spar-blind-item" key={i}>{b}</p>
                  ))}
                </div>
              )}
              {verdict.summary && (
                <div className="spar-verdict-summary">{verdict.summary}</div>
              )}
              <div className="spar-verdict-actions">
                <button className="ds-btn-primary" onClick={() => { setStatus("input"); setExchanges([]); setRationale(""); setVerdict(null); setDefenseInput(""); }}>
                  New session
                </button>
...
function LegalPage({ page, onBack, goPage }) {
  ...
  return (
    <div className="ds-legal">
      ...
      {page === "privacy" && <>
        <h1 className="ds-legal-title">Privacy Policy</h1>
        <span className="ds-legal-date">Effective date: May 2026</span>
        <h2>What this is</h2>
        <p>Design Sparring is a tool for stress-testing design decisions. We collect the minimum information necessary to provide the service. This policy explains what we collect, why, and what you can do about it.</p>
        <h2>What we collect</h2>
        <p>We collect your email address when you create an account. That's it. We use it solely for authentication via magic links — no password, no personal profile. On the paid tier, we store your sparring session data (your inputs, counter-arguments, and verdicts) for up to 30 days. On the free tier, no account or session data is retained.</p>
        <h2>How we use your data</h2>
        <ul>
          <li>To send you a magic link for authentication</li>
          <li>To associate your sessions with your account (paid tier only)</li>
        ...
        <p>Your account and email are retained until you delete your account. Paid-tier session data is deleted automatically after 30 days. You can delete your account at any time from the account menu, which permanently removes all associated data.</p>
        <h2>Third parties</h2>
        <p>We use a transactional email provider to deliver magic links. Paid accounts use a third-party payment processor — we never see or store your card details. We do not sell, rent, or share your data with any other third parties. We do not use advertising networks or tracking pixels.</p>
        <hr className="ds-legal-rule" />
        <h2>Cookies</h2>
        <p>We use only essential session cookies required for authentication to function. See our <button style={{background:"none",border:"none",cursor:"pointer",textDecoration:"underline",fontSize:"inherit",fontFamily:"inherit",padding:0,fontWeight:400}} onClick={() => goPage("cookies")}>Cookie Policy</button> for details.</p>
        <h2>Your rights</h2>
        <p>You can request a copy of your data, correct your email address, or delete your account entirely at any time. To exercise any of these rights, use the account settings or contact us directly.</p>
        <h2>Contact</h2>
        <p>Questions? <a href="mailto:privacy@designsparring.com">privacy@designsparring.com</a></p>
      </>}
      {page === "terms" && <>
        <h1 className="ds-legal-title">Terms of Service</h1>
        <span className="ds-legal-date">Effective date: May 2026</span>
        <h2>Acceptance</h2>
        <p>By creating an account or using Design Sparring, you agree to these terms. If you don't agree, don't use the service.</p>
        <h2>What the service is</h2>
        <p>Design Sparring provides AI-generated counter-arguments and structured scoring for design decisions you submit. It is a decision-support tool, not professional design, legal, or business advice. The AI opponent is intentionally adversarial — that's the point.</p>
        <h2>Accounts</h2>
        <p>You must be at least 16 years old to create an account. We use magic-link authentication — keep your email account secure. The free tier does not require an account.</p>
        <h2>Free tier</h2>
        <p>The free tier provides up to 3 sparring sessions per day, limited to one round per session, with no account required. No session data is retained. Free tier access may be adjusted at any time with reasonable notice.</p>
        <h2>Paid tier and billing</h2>
        <p>Paid subscriptions are billed monthly. You can cancel at any time — cancellation takes effect at the end of the current billing period. No refunds for partial months. Failed payments result in downgrade after a 7-day grace period.</p>
        <h2>Acceptable use</h2>
        <p>You agree not to submit content that is illegal, harmful, or infringes the rights of others. You agree not to scrape or abuse the service in ways that degrade performance for others. We reserve the right to suspend accounts that violate these terms.</p>
        <h2>Your content</h2>
        <p>You retain ownership of any content you submit. We do not use your submitted content to train AI models or share it with third parties.</p>
        <h2>Disclaimers</h2>
        <p>Design Sparring is provided "as is." AI counter-arguments are intentionally one-sided. Do not make irreversible decisions based solely on output from this tool.</p>
        <h2>Limitation of liability</h2>
        <p>To the maximum extent permitted by law, Design Sparring is not liable for any indirect or consequential damages. Our total liability for any claim will not exceed the amount you paid us in the preceding 12 months.</p>
        <h2>Contact</h2>
        <p><a href="mailto:legal@designsparring.com">legal@designsparring.com</a></p>
      </>}
      {page === "cookies" && <>
        <h1 className="ds-legal-title">Cookie Policy</h1>
        <span className="ds-legal-date">Effective date: May 2026</span>
        <h2>What cookies are</h2>
        <p>Cookies are small text files stored in your browser when you visit a website. We use them only for authentication and your stated cookie preferences.</p>
        <h2>What we use</h2>
        <ul>
          <li><strong>Session cookie</strong> — Set when you sign in. Keeps you authenticated. Expires when you sign out.</li>
          <li><strong>CSRF token</strong> — A security cookie protecting against cross-site request forgery.</li>
          <li><strong>Cookie consent</strong> — Stores your preference so we don't ask again every visit.</li>
        </ul>
        <h2>What we don't use</h2>
        <p>No advertising cookies. No cross-site tracking. No third-party analytics. No fingerprinting.</p>
        <h2>How to manage cookies</h2>
        <p>You can block or delete cookies through your browser settings. Blocking essential cookies will prevent you from signing in.</p>
        <h2>Contact</h2>
        <p><a href="mailto:privacy@designsparring.com">privacy@designsparring.com</a></p>
      </>}
    </div>
  );
}
...
export default function LandingPage() {
  ...
  const handleAuth = () => {
    ...
    if (modal === "signup") setAuthStep("sent");
    else { setUser({ email: email.trim(), plan: "free" }); showToast("Welcome back."); closeModal(); }
  };
  const simulateMagicLink = () => { setUser({ email: email.trim(), plan: "free" }); showToast("Account created. Welcome to Design Sparring."); closeModal(); };
  const handleLogout = () => { setUser(null); setAcctOpen(false); showToast("Signed out."); };
  const handleUpgrade = () => { setUser(u => ({ ...u, plan: "paid" })); setAcctOpen(false); showToast("Upgraded to Sparring Partner."); };
  const handleDelete = () => {
    if (!deleteStep) setDeleteStep(true);
    else { setUser(null); closeModal(); showToast("Account deleted."); }
  };
    ...
    <div className="ds" onClick={() => acctOpen && setAcctOpen(false)}>
          ...
          {user ? (
            <div className="ds-account-wrap" onClick={e => e.stopPropagation()}>
              <button className="ds-account-btn" onClick={() => setAcctOpen(!acctOpen)}>
                <span className="ds-dot" /> Your account
              </button>
              {acctOpen && (
                <div className="ds-dropdown">
                  <div className="ds-dropdown-meta">
                    <span className={`ds-tag${user.plan === "paid" ? " ds-tag-paid" : ""}`}>{user.plan === "paid" ? "Sparring Partner" : "Free plan"}</span>
                    {user.plan === "free" && <button className="ds-acct-upgrade-btn" onClick={handleUpgrade}>Upgrade</button>}
                  </div>
      ...
      {page === "spar" && (
        <SparringInterface
          user={user}
          onExit={() => goPage("home")}
          onSignup={() => { goPage("home"); openModal("signup"); }}
          showToast={showToast}
        />
      )}
          ...
          <div className="ds-hero-inner">
            ...
            <div className="ds-card-mockup">
              <div className="ds-card-top">
                <div className="ds-card-lbl">Your rationale — Round 01</div>
                <div className="ds-card-rounds">
                  <div className="ds-card-round-pip ds-card-round-pip-on" />
                  <div className="ds-card-round-pip" />
                  <div className="ds-card-round-pip" />
                </div>
              </div>
              ...
              <div className="ds-card-foot">
                <span className="ds-round-lbl">Round 01 / 03</span>
                <span className="ds-defend-lbl">Defend your position →</span>
              </div>
            </div>
          </div>
          ...
          <div className="ds-section-inner">
            ...
            <div className="ds-scores">
              {SCORES_PREVIEW.map(s => (
                <div className="ds-score-row" key={s.label}>
            ...
            <div className="ds-pricing">
              <div className="ds-plan">
                ...
                <ul className="ds-plan-list">
                  <li>3 sessions per day</li><li>1 round per session</li><li>No sign-in required</li><li>No session memory</li><li>No verdict scoring</li>
                </ul>
              ...
              <div className="ds-plan ds-plan-dark">
                ...
                <ul className="ds-plan-list">
                  <li>Unlimited sessions</li><li>Full 3-round format</li><li>30-day session history</li><li>Scored verdict on every session</li>
                </ul>
      ...
      {modal && (
        <div className="ds-overlay" onClick={closeModal}>
          <div className="ds-modal" onClick={e => e.stopPropagation()}>
            ...
            {modal === "login" && <>
              <h2 className="ds-modal-title">Sign in</h2>
              <p className="ds-modal-sub">Enter your email. We'll send a link.</p>
              ...
              <button className="ds-btn-primary ds-btn-full" onClick={handleAuth}>Continue</button>
              <p className="ds-modal-switch">New here? <button className="ds-text-btn" onClick={() => openModal("signup")}>Create an account</button></p>
            </>}

            {modal === "signup" && authStep === "form" && <>
              <h2 className="ds-modal-title">Create account</h2>
              <p className="ds-modal-sub">No password. Just an email. Required for the paid plan.</p>
              <input className="ds-input" type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && handleAuth()} autoFocus />
              <button className="ds-btn-primary ds-btn-full" onClick={handleAuth}>Create account</button>
              <p className="ds-modal-switch">Already have one? <button className="ds-text-btn" onClick={() => openModal("login")}>Sign in</button></p>
            </>}

            {modal === "signup" && authStep === "sent" && <>
              <h2 className="ds-modal-title">Check your email</h2>
              <p className="ds-modal-sub">A link is on its way to <strong>{email}</strong></p>
              <div className="ds-magic-box">
                <div className="ds-magic-hint">Demo — simulate clicking the link</div>
                <button className="ds-btn-primary ds-btn-full" onClick={simulateMagicLink}>Open magic link →</button>
              </div>
            </>}

            {modal === "history" && <>
              <h2 className="ds-modal-title">Session history</h2>
              <div className="ds-acct-plan">
                <span className="ds-acct-plan-label ds-acct-plan-paid">Sparring Partner</span>
                <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--muted)", letterSpacing: "0.08em" }}>30-day window</span>
              </div>
              <div className="ds-session-lbl">Recent sessions</div>
              <div className="ds-session-list">
                {SAMPLE_SESSIONS.map(s => (
                  <div className="ds-session-row" key={s.id}>
                    <div className="ds-session-info">
                      <span className="ds-session-title">{s.title}</span>
                      <span className="ds-session-date">{s.date}</span>
                    </div>
                    <Pips score={s.score} size="sm" />
                  </div>
                ))}
              </div>
              <div className="ds-acct-danger-zone">
                <button className="ds-dropdown-item ds-danger-item" style={{ padding: "10px 0" }} onClick={() => { closeModal(); openModal("delete"); }}>Delete account</button>
              </div>
            </>}
